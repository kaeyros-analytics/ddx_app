...
user_location_ui <- function(id) {
  ...
  tagList(
    tags$div(
      class = "map-plot1",
      shinycssloaders::withSpinner(plotly::plotlyOutput(outputId = ns("user_location_plot")), type = 1, color = 'blue')
    ) # end tags$div
  ) # end tagList
  ...
  output$user_location_plot <- renderPlotly({
    ...
    plot_user_location_stack_ <- plotly::plot_ly(data_location_purchased, x = ~month_year,
      ...
      layout(title = "",  barmode="stack", bargap=0.3,
             legend = list(x = 100, y = 0.45, title=list(color= "blue", text='<b>User location</b>')),
