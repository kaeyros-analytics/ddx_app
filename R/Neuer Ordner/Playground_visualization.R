...
train_df_testdata <- df_filter %>%
  filter(date >= "2001-01-31" & date <= "2015-12-31")
...
library(modeltime)
library(timetk)
# install install_gluonts() before library(modeltime.gluonts)
...
train_df_testdata %>%
  ...
  ggplot2::ggplot(train_df_testdata, aes(x = date, y = lot_health_index)) +
  ggplot2::geom_line()
...
head(tsibble_train_df_testdata, 5)

essai <- tsibble::fill_gaps(tsibble_train_df_testdata)

library(forecast)
mesure_ts <- ts(tsibble_train_df_testdata$lot_health_index, start = min(tsibble_train_df_testdata$date), 
                end = max(tsibble_train_df_testdata$date),frequency = 4)

forecast::autoplot(mesure_ts, facets = TRUE) +
  scale_x_continuous(breaks = labels = date_format("%m-%Y"))

autoplot(hydrometeo_ts, facets = TRUE) +
  scale_x_continuous(labels=Date, breaks = 1998:2008)


fpp3_model_table <- essai %>%
  fabletools::model(
    ## Model 1: Naive ----
    naive_mod = fable::NAIVE(log(lot_health_index + 1)))

str(train_df_testdata)
