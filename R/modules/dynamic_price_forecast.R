  ...
  output$dynamic_price <- shiny::renderPlot({
    ...
    if(input$selected_forecast_period == 3){
      plot_dynamic_price <- ggplot() + 
        geom_line(data = as.data.frame(dynamic_price_inv_dataframe[1:n_3,]), aes(y = get("dynamic_price_inv_dataframe[1:n_3, ]"), 
                                                                                 x = seq(1, n_3)), color = "#59CE8F") +
      ...
      plot_dynamic_price <- ggplot() + 
        geom_line(data = as.data.frame(dynamic_price_inv_dataframe[1:n_6,]), aes(y = get("dynamic_price_inv_dataframe[1:n_6, ]"), 
                                                                                 x = seq(1, n_6)), color = "#59CE8F") +
        geom_line(data = as.data.frame(dynamic_price_inv_dataframe[n_5:nbr_row,]), aes(y = get("dynamic_price_inv_dataframe[n_5:nbr_row, ]"), 
      ...
      plot_dynamic_price <- ggplot() + 
      geom_line(data = as.data.frame(dynamic_price_inv_dataframe[1:n_12,]), aes(y = get("dynamic_price_inv_dataframe[1:n_12, ]"), 
                                                                                x = seq(1, n_12)), color = "#59CE8F") +
      geom_line(data = as.data.frame(dynamic_price_inv_dataframe[n_11:nbr_row,]), aes(y = get("dynamic_price_inv_dataframe[n_11:nbr_row, ]"), 
