...
dynamic_price_server <- function(input, output, session) {
  ...
  output$dynamic_price <- plotly::renderPlotly({
    ...
    plot_dynamic_price <- plot_dynamic_price %>% add_trace(data = as.data.frame(dynamic_price_inv_dataframe[1:n_1,]), x = ~seq(1, n_1), 
                                                           y = ~get("dynamic_price_inv_dataframe[1:n_1, ]"),
                                                           mode = 'lines', line = list(color = "#17B169"),
                                                           hovertext = paste('Time: ',"<b>",seq(1, n_1), "</b>",
                                                                             '<br>Dynamic price:',"<b>",
    ...
    plot_dynamic_price <- plot_dynamic_price %>% add_trace(data = as.data.frame(dynamic_price_inv_dataframe[n_2:nbr_row,]), 
                                                           x = ~seq(n_2, nbr_row), y = ~get("dynamic_price_inv_dataframe[n_2:nbr_row, ]"),
                                                           mode = 'lines', line = list(color = "#ff0000"),
                                                           hovertext = paste('Time: ',"<b>",seq(n_2, nbr_row), "</b>",
                                                                             '<br>Dynamic price:',"<b>",
    ...
    if(average_event()){ 
      plot_dynamic_price <- plot_dynamic_price %>% add_lines(data =filter_action(), x = seq(1, nrow(filter_action())), y = filter_action()$fixed_price,  
                                                             line = list(color = "#E8AA42"), hoverinfo = 'text',
                                                             inherit = FALSE, showlegend = FALSE)
      plot_dynamic_price <- plot_dynamic_price %>% add_annotations(x = 50, y = filter_action()$fixed_price[1] + 0.028, 
                                                                   text = paste("fixed premium = ", round(filter_action()$fixed_price[1], 2), sep = ""),
                                                                   showarrow  = F)
      # plot_dynamic_price <- plot_dynamic_price + 
